(function () {
  ...
  const seedState = {
    ...
    resume: {
      fileName: "",
      extractedSkills: ["Vue.js", "Figma", "Python"],
    ...
    employerSettings: {
      name: "IT Department",
      summary: "The IT Department hires student assistants for technical support, systems work, and internal tools.",
      defaultJobType: "Part-time",
      defaultWorkSetup: "Flexible",
    ...
    users: [
      {
        ...
        yearLevel: "2nd Year",
        section: "2-B",
        about: "2nd-year Computer Science student passionate about building user-centered web applications. Looking for part-time opportunities or internships in UI/UX design and web development within Gordon College.",
      ...
      },
      {
        ...
        yearLevel: "3rd Year",
        section: "3-A",
        about: "BSIT student focused on practical web and systems work inside campus departments.",
        skills: ["Vue.js", "REST APIs", "JavaScript", "HTML/CSS", "Troubleshooting"],
        preferences: ["Open to opportunities", "Flexible hours"],
        experience: [{ title: "Laboratory Assistant", org: "IT Department", dates: "2024", desc: "Maintained lab equipment and student support tools." }],
      },
      {
        id: "student-maria",
        role: "student",
        ...
        experience: [{ title: "Design Volunteer", org: "Student Affairs", dates: "2024", desc: "Created posters and campaign assets for campus activities." }],
      },
      {
        id: "student-rey",
        role: "student",
      ...
      { id: "employer-it", role: "employer", firstName: "IT", lastName: "Department", email: "itdept@gordoncollege.edu.ph", password: "employer123", department: "IT Department" },
      { id: "admin-1", role: "admin", firstName: "Platform", lastName: "Admin", email: "admin@gordoncollege.edu.ph", password: "admin123", department: "CCS" },
    ],
    jobs: [
      { id: "job-uiux", title: "UI/UX Design Intern", department: "College of Computer Studies", type: "Part-time", setup: "On-site", schedule: "3 days/week", description: "Assist faculty in designing UI prototypes and student research flows.", skills: ["Figma", "Adobe XD", "Prototyping", "User Research"], slots: 2, employerId: "employer-it", postedAt: "2026-04-06", status: "active", matchOverrides: { "student-allyana": 94 } },
      { id: "job-webdev", title: "Web Development Assistant", department: "IT Department", type: "Part-time", setup: "Flexible", schedule: "Mon-Fri, flexible hours", description: "Maintain internal web tools, fix UI bugs, and support REST API work.", skills: ["Vue.js", "Django", "REST APIs", "Python", "HTML/CSS"], slots: 2, employerId: "employer-it", postedAt: "2026-04-02", status: "active", matchOverrides: { "student-allyana": 81, "student-juan": 78, "student-maria": 72, "student-rey": 69, "student-katrina": 58 } },
      { id: "job-graphic", title: "Graphic Design Volunteer", department: "Student Affairs", type: "Volunteer", setup: "On-site", schedule: "Weekends only", description: "Produce posters and digital assets for student affairs campaigns.", skills: ["Canva", "Photoshop", "Layout"], slots: 1, employerId: "employer-it", postedAt: "2026-04-07", status: "active", matchOverrides: { "student-allyana": 73 } },
      { id: "job-content", title: "Content Creator — Social Media", department: "Marketing Office", type: "Part-time", setup: "Remote", schedule: "10 hours/week", description: "Create social posts, short-form captions, and basic edited reels.", skills: ["Copywriting", "Video editing", "Canva"], slots: 1, employerId: "employer-it", postedAt: "2026-04-05", status: "active", matchOverrides: { "student-allyana": 68 } },
      { id: "job-library", title: "Library Digital Assistant", department: "Academic Services", type: "Part-time", setup: "On-site", schedule: "Afternoons", description: "Support digitization, spreadsheets, and catalog maintenance.", skills: ["Data entry", "MS Office", "Documentation"], slots: 2, employerId: "employer-it", postedAt: "2026-04-04", status: "active", matchOverrides: { "student-allyana": 61 } },
      { id: "job-ai-lab", title: "Research Assistant — AI Lab", department: "CCS Research Center", type: "Internship", setup: "On-site", schedule: "Flexible", description: "Support small ML experiments, documentation, and dataset cleanup.", skills: ["Python", "Machine Learning", "Documentation"], slots: 1, employerId: "employer-it", postedAt: "2026-04-03", status: "active", matchOverrides: { "student-allyana": 77 } },
    ],
    applications: [
  ...
  function scoreJob(student, job) {
    ...
    const overlap = job.skills.filter((skill) => studentSkills.includes(skill.toLowerCase())).length;
  ...
  function suggestedSkill(student) {
    const skillSet = new Set(student.skills.map((skill) => skill.toLowerCase()));
  ...
  function navigateFor(user) {
    if (user.role === "employer") window.location.href = "hire_gcians_employer_dashboard.html";
    else if (user.role === "admin") window.location.href = "hire_gcians_admin.html";
  ...
  function syncChrome() {
    ...
    if (user.role === "employer") {
      document.querySelectorAll(".s-dept").forEach((node) => (node.textContent = "Gordon College"));
    }
  }

  function syncRoleActions() {
    const user = currentUser();
    const employerVisible = user.role === "employer" || user.role === "admin";
  ...
  function ensurePageUser() {
    const user = currentUser();
    if ((PAGE.startsWith("employer") || PAGE === "department-profile" || PAGE === "hired-students") && user.role !== "employer") {
    ...
    if (signupButton) {
      signupButton.addEventListener("click", () => {
        ...
        const user = {
          ...
          section: "1-A",
          department: document.getElementById("signup-department")?.value || "IT Department",
          about: role === "student" ? "New student account." : "New employer account.",
          skills: role === "student" ? ["HTML/CSS"] : [],
          preferences: role === "student" ? ["Open to opportunities"] : [],
          experience: [],
        };
        state.users.push(user);
        if (role === "student") state.saved[user.id] = [];
        setSession(user.id);
        saveState();
        navigateFor(user);
      });
  ...
  function renderStudentDashboard() {
      ...
      recs.innerHTML = jobs.slice(0, 4).map((job) => `
        <div class="job-card">
          <div>
            ...
            <div class="job-dept">${escapeHtml(job.department)} · ${escapeHtml(job.type)} · ${escapeHtml(job.setup)}</div>
    ...
    if (skillsWrap) skillsWrap.innerHTML = user.skills.slice(0, 6).map((skill) => `<span class="skill-tag">${escapeHtml(skill)}</span>`).join("");
    const completeness = Math.min(95, 55 + user.skills.length * 2 + user.experience.length * 8);
    ...
    setText("#student-dashboard-tip", `Add "${suggestedSkill(user)}" to your skills to improve one of your top matches.`);
  }

  function renderJobListing() {
    const user = currentUser();
    const search = (document.getElementById("job-search-input")?.value || "").trim().toLowerCase();
    const filters = state.ui.jobFilters || { types: [], departments: [], setups: [], minScore: 0 };
    const jobs = recommendedJobs(user).filter((job) => {
      const matchesSearch = `${job.title} ${job.department} ${job.skills.join(" ")}`.toLowerCase().includes(search);
      const matchesType = !filters.types.length || filters.types.includes(job.type);
      const matchesDepartment = !filters.departments.length || filters.departments.includes(job.department);
      ...
      return matchesSearch && matchesType && matchesDepartment && matchesSetup && matchesScore;
    });
    ...
    if (filterPanel) {
      ...
      const departmentOptions = Array.from(new Set(allJobs.map((job) => job.department)));
      ...
      filterPanel.innerHTML = `
        ...
        <div class="filter-group">
          ...
          ${renderOptions("departments", departmentOptions, filters.departments || [])}
        </div>
        <div class="filter-group">
    ...
    if (list) {
      list.innerHTML = jobs.map((job) => `
        <div class="job-row ${job.id === selectedId ? "selected" : ""}" data-job-id="${job.id}">
          <div class="jr-top">
            <div><div class="jr-title">${escapeHtml(job.title)}</div><div class="jr-dept">${escapeHtml(job.department)}</div></div>
    ...
    if (detailPanel) {
      ...
      detailPanel.innerHTML = `
        ...
        <div class="dp-dept">${escapeHtml(job.department)} · Posted ${shortDate(job.postedAt)}</div>
        <div class="dp-match-box">
          ...
          <div style="font-size:0.72rem;color:var(--gc-green);margin-top:0.25rem">${score >= 80 ? "Excellent fit based on your skills" : score >= 65 ? "Solid fit with room to improve" : "Needs more matching skills"}</div>
        </div>
        ...
        <div class="dp-section">
          ...
          ${job.skills.map((skill) => `<div class="skill-match-row"><span class="skill-match-name">${escapeHtml(skill)}</span><span class="${userSkills.has(skill.toLowerCase()) ? "skill-match-yes" : "skill-match-no"}">${userSkills.has(skill.toLowerCase()) ? "✓ You have this" : "Not listed yet"}</span></div>`).join("")}
        </div>
        ...
        return `
          <div class="app-card" data-status="${app.status}">
            <div>
              ...
              <div class="app-dept">${escapeHtml(job.department)} · ${escapeHtml(job.type)} · ${escapeHtml(job.setup)}</div>
    ...
    if (grid) {
      grid.innerHTML = jobs.map((job) => `
        <div class="saved-card">
          ...
          <div class="sc-dept">${escapeHtml(job.department)} · ${escapeHtml(job.type)} · ${escapeHtml(job.setup)}</div>
      ...
      grid.querySelectorAll("[data-view-job]").forEach((button) => button.addEventListener("click", () => {
        ...
        saveState();
        window.location.href = "hire_gcians_job_listing.html";
      }));
  ...
  function renderStudentProfile() {
    ...
    if (skills) skills.innerHTML = `${user.skills.map((skill) => `<span class="skill-chip">${escapeHtml(skill)}</span>`).join("")}<span class="add-skill">+ Add more</span>`;
    ...
    setText("#profile-ai-tip", `Add "${suggestedSkill(user)}" to your skills — it improves your best-fit openings.`);
    const addSkill = document.getElementById("profile-add-skill");
    if (addSkill) {
      addSkill.onclick = () => {
        const skill = suggestedSkill(user);
  ...
  function renderForEmployersPage() {
    ...
    const departments = new Set(activeJobs.map((job) => job.department)).size;
    if (stats) {
      stats.innerHTML = `<div class="stat"><div class="stat-num">${activeJobs.length}</div><div class="stat-label">Open campus roles</div></div><div class="stat"><div class="stat-num">${applicants}</div><div class="stat-label">Applications in demo state</div></div><div class="stat"><div class="stat-num">${departments}</div><div class="stat-label">Departments represented</div></div>`;
    }
    if (jobsHost) {
      jobsHost.innerHTML = activeJobs.slice(0, 3).map((job, index) => `<div class="match-card" ${index ? 'style="background:#fff;border:0.5px solid #C0DD97"' : ""}><div class="match-header"><div class="match-title">${escapeHtml(job.title)}</div><div class="match-pct">${applicationsForJob(job.id).length} applicants</div></div><div class="match-sub">${escapeHtml(job.department)} · ${escapeHtml(job.type)} · ${escapeHtml(job.setup)}</div><div class="match-tags">${job.skills.slice(0, 3).map((skill) => `<span class="tag">${escapeHtml(skill)}</span>`).join("")}</div></div>`).join("");
    }
  }
  ...
  function renderAboutPage() {
    ...
    if (features) {
      features.innerHTML = [`AI-style job matching from profile skills`, `Employer dashboard for postings and applicants`, `Resume workflow aimed at PDF skill extraction`, `Navigation across student, employer, and admin routes`].map((item) => `<div class="panel-row">${escapeHtml(item)}</div>`).join("");
    }
  }
  ...
  function renderAuthPage() {
    ...
    if (cards) {
      ...
      cards.innerHTML = jobs.map((job, index) => `<div class="lp-card"><div class="lp-card-dot" style="${index === 1 ? "background:#639922" : index === 2 ? "background:#97C459" : ""}"></div><div><div class="lp-card-title">${escapeHtml(job.title)}</div><div class="lp-card-sub">${escapeHtml(job.department)} · ${escapeHtml(job.type)}</div></div><div class="lp-pct" style="${index === 1 ? "background:#639922" : index === 2 ? "background:#97C459;color:#1a2e0a" : ""}">${job.score}%</div></div>`).join("");
    }
  }

  function analyzeResumeFile(file) {
    const user = currentUser();
    const fileName = file ? file.name : state.resume.fileName || "resume.pdf";
    const extracted = Array.from(new Set(user.skills.slice(0, 6).concat([suggestedSkill(user), "Communication"]))).slice(0, 8);
    const quantifiable = [
    ...
    state.resume = {
      ...
      quantifiable,
      summary: `Demo AI analysis for ${fileName}: strongest signals are ${extracted.slice(0, 3).join(", ")}. In production, the app should extract PDF text first, then use AI to identify explicit skills and whether achievements are quantifiable.`,
    };
    if (!user.skills.includes(suggestedSkill(user))) user.skills.push(suggestedSkill(user));
    saveState();
  }
  ...
  function renderSkillsResume() {
    ...
    if (status) status.textContent = state.resume.fileName ? `Current file: ${state.resume.fileName}` : "No PDF uploaded yet.";
    if (grid) grid.innerHTML = state.resume.extractedSkills.map((skill) => `<span class="skill-chip">${escapeHtml(skill)}</span>`).join("");
    if (quant) quant.innerHTML = state.resume.quantifiable.map((item) => `<div class="quant-row">${escapeHtml(item)}</div>`).join("");
    if (summary) summary.textContent = state.resume.summary;
    if (recommendation) recommendation.textContent = "This browser-only demo can infer a skills preview, but real PDF scanning should run through a backend extraction pipeline so the AI can classify skills and detect measurable outcomes reliably.";
  ...
  function renderPublicProfile() {
    ...
    if (skills) skills.innerHTML = user.skills.map((skill) => `<span class="skill-chip">${escapeHtml(skill)}</span>`).join("");
  ...
  function renderEmployerPosting() {
    ...
    const department = document.getElementById("posting-department");
    ...
    state.ui.postingDraft = state.ui.postingDraft || {
      title: "",
      department: employerSettings.name || "IT Department",
    ...
    department.value = state.ui.postingDraft.department;
    ...
    function syncDraft() {
      ...
      setText("#posting-preview-dept", `${department.value} · Posted just now`);
      ...
      const predictions = students().map((student) => ({ student, score: scoreJob(student, { skills: state.ui.postingSkills, matchOverrides: {}, title: title.value, department: department.value, description: description.value }) })).sort((a, b) => b.score - a.score);
      ...
      if (estimate) estimate.innerHTML = `Based on your listed skills, approximately <strong style="color:var(--gc-green)">${strongMatches} students</strong> on the platform have a match score above 70% for this role.`;
    ...
    [title, department, type, setup, schedule, description, slots].forEach((node) => node && node.addEventListener("input", syncDraft));
    if (skillInput) {
    ...
    document.querySelector(".submit-btn")?.addEventListener("click", () => {
      ...
      saveState();
      window.location.href = "hire_gcians_employer_dashboard.html";
    });
    syncDraft();
  }
  ...
  function renderDepartmentProfile() {
    const jobs = jobsForEmployer(currentUser().id);
    setText("#department-profile-summary", state.employerSettings.summary);
    const jobsNode = document.getElementById("department-profile-jobs");
  ...
  function renderEmployerSettings() {
    ...
    const publicDepartment = document.getElementById("employer-setting-public");
    ...
    if (publicDepartment) publicDepartment.checked = settings.publicDepartment;
    document.getElementById("employer-settings-save")?.addEventListener("click", () => {
      state.employerSettings = {
        name: name.value.trim() || "IT Department",
        summary: summary.value.trim() || "Department summary pending.",
        ...
        publicDepartment: !!publicDepartment.checked,
      };
      if (state.ui.postingDraft) {
        state.ui.postingDraft.department = state.employerSettings.name;
  ...
  function renderEmployerApplicants() {
    ...
    if (list) {
      list.innerHTML = visible.map((item) => `<div class="ap-card ${item.app.id === state.ui.employerSelectedApplicationId ? "selected" : ""}" data-application-id="${item.app.id}"><div class="ap-top"><div class="ap-av-sm">${initials(item.student)}</div><div class="ap-info"><div class="ap-name">${escapeHtml(item.student.firstName)} ${escapeHtml(item.student.lastName)}</div><div class="ap-course">${escapeHtml(item.student.program)} ${escapeHtml(item.student.section)} · Applied ${shortDate(item.app.appliedAt)}</div></div><div class="ap-pct-sm ${pctClass(item.score)}">${item.score}%</div></div><div class="ap-chips">${item.student.skills.slice(0, 3).map((skill) => `<span class="ap-chip-sm">${escapeHtml(skill)}</span>`).join("")}</div></div>`).join("");
    ...
    if (!selected) return;
    const hasCount = selected.jobSkillsMatchCount || selectedJob.skills.filter((skill) => selected.student.skills.map((s) => s.toLowerCase()).includes(skill.toLowerCase())).length;
    ...
    if (skillBreakdown) {
      const set = new Set(selected.student.skills.map((skill) => skill.toLowerCase()));
      skillBreakdown.innerHTML = selectedJob.skills.map((skill) => `<div class="skill-row"><span>${escapeHtml(skill)}</span><span class="${set.has(skill.toLowerCase()) ? "skill-match" : "skill-miss"}">${set.has(skill.toLowerCase()) ? "✓ Has this skill" : "Not listed"}</span></div>`).join("");
    }
  ...
  function initAdminPage() {
    ...
    if (usersTable) {
      usersTable.innerHTML = state.users.slice(0, 4).map((user, index) => `<div class="t-row users-grid"><div class="u-info"><div class="u-av" style="background:${index % 2 ? "#639922" : "#3B6D11"}">${initials(user)}</div><div><div class="u-name">${escapeHtml(user.firstName)} ${escapeHtml(user.lastName)}</div><div class="u-course">${escapeHtml(user.role === "student" ? `${user.program} ${user.section}` : user.role === "employer" ? "Employer account" : "Admin account")}</div></div></div><div><span class="role-pill ${user.role === "student" ? "rp-student" : "rp-employer"}">${escapeHtml(user.role[0].toUpperCase() + user.role.slice(1))}</span></div><div style="color:rgba(192,221,151,0.5)">${escapeHtml(user.program || "—")}</div><div style="color:rgba(192,221,151,0.5)">${shortDate("2026-04-01")}</div><div class="${index === 3 ? "status-inactive" : "status-active"}"><div class="s-dot" style="background:${index === 3 ? "rgba(192,221,151,0.2)" : "#97C459"}"></div>${index === 3 ? "Inactive" : "Active"}</div></div>`).join("");
    }
    ...
    if (jobsTable) {
      jobsTable.innerHTML = state.jobs.filter((job) => job.status === "active").slice(0, 4).map((job) => {
        ...
        return `<div class="t-row jobs-grid"><div style="color:#C0DD97;font-weight:500">${escapeHtml(job.title)}</div><div style="color:rgba(192,221,151,0.5)">${escapeHtml(job.department)}</div><div style="color:#97C459">${applicants.length}</div><div style="color:${avgScore >= 70 ? "#97C459" : "rgba(192,221,151,0.4)"}">${avgScore}%</div><div><span style="font-size:0.7rem;color:#97C459">● Active</span></div></div>`;
      }).join("");
    }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (PAGE === "department-profile") renderDepartmentProfile();
